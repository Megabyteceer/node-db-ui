...
serverOn(E._languages.afterCreate, async (data, _userSession) => {
	shouldBeAdmin();
...
serverOn(E._languages.beforeUpdate, async (_currentData, newData, _userSession) => {
	if (newData.hasOwnProperty('code')) {
		throwError('Cant change \'code\' of language.');
	}
	reloadMetadataSchedule();
});
serverOn(E._languages.beforeDelete, async (_data, _userSession) => {
