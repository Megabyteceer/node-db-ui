...
import { NodeEventsHandlers, reloadMetadataSchedule } from "../core/descript-node";
...
const handlers: NodeEventsHandlers = {
	...
	afterCreate: async function(data: RecordDataWrite, userSession: UserSession) {
		...
		if(data.isDocument && !data.staticLink) {
			...
			CONSTRAINT creator_id_${data.tableName} FOREIGN KEY (_userID) REFERENCES _users (id) ON DELETE RESTRICT ON UPDATE RESTRICT,
			...
			const insertedId = (await mysqlExec("INSERT INTO \`" + data.tableName + "\` SET status=0, _userID=0") as mysqlInsertResult).insertId;
			await mysqlExec("UPDATE \`" + data.tableName + "\` SET id=0 WHERE id=" + insertedId);

			//create default fields
			const mainFieldQ = `INSERT INTO _fields
				(node_fields_linker, status, \`show\`,          prior, fieldType,       fieldName, selectFieldName, name,           description, maxLength, requirement, unique, _userID, forSearch, noStore) VALUES
			...
			if(data.addCreatedByFiled) {
				...
				(node_fields_linker, status, \`show\`,                                 prior, fieldType,       fieldName,  selectFieldName, name,                   description, maxLength, requirement, unique, _userID, forSearch, noStore, nodeRef) VALUES
			...
			if(data.createUserFld) {
				const createdByQ = `INSERT INTO _fields
				(node_fields_linker, status, \`show\`,                                 prior, fieldType,       fieldName,  selectFieldName,  name,           description, maxLength, requirement, unique, _userID, forSearch, noStore, nodeRef,          icon) VALUES
				(${createdID},       1,        ${VIEW_MASK_LIST | VIEW_MASK_READONLY}, 4,     ${FIELD_7_Nto1}, '_userID', '_users',        '${L('Owner')}', '',           0,        0,           0,      0,        1,         0,       ${NODE_ID_USERS}, 'avatar');`;
				await mysqlExec(createdByQ);
			}
		}
