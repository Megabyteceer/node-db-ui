...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../admin/field-admin.js", "../admin/node-admin.js", "../left-bar.js", "../user.js", "../utils.js", "./form-full.js", "./form-item.js", "./form-mixins.js"], factory);
    }
    ...
    var form_full_js_1 = require("./form-full.js");
    var form_item_js_1 = require("./form-item.js");
    ...
    var List = /** @class */ (function (_super) {
        ...
        List.prototype.renderEditableList = function () {
                ...
                for (var i = 0; i < data.items.length; i++) {
                    (function () {
                        ...
                        if (!item.__deleted_901d123f) {
                            ...
                            lines.push(R.span({ key: utils_js_1.UID(item) + 'btns', className: 'btns' }, btns));
                        }
                    })();
                }
            }
            ...
            var createBtn;
            if (node.canCreate) {
                createBtn = R.div(null, R.button({ title: utils_js_1.L('ADD', (node.creationName || node.singleName)), className: 'clickable toolbtn create-btn', onClick: function () { data.items.push({}); _this.forceUpdate(); } }, utils_js_1.renderIcon('plus')));
            }
            return R.div(null, nodeAdmin, lines, createBtn);
        };
        ...
        List.prototype.renderList = function () {
            ...
            if (!this.props.omitHeader) {
                ...
                if (node.canCreate && !this.props.preventCreateButton && !this.filters.preventCreateButton && !this.state.preventCreateButton) {
                    if (this.isSlave()) {
                        ...
                        createButton = R.button({ className: 'clickable create-button', onClick: function () { utils_js_1.createRecord(node.id, filters); } }, utils_js_1.renderIcon('plus'), ' ' + utils_js_1.L('CREATE') + ' ' + (node.creationName || node.singleName));
                    }
                }
                var searchPanel;
                if (!this.props.hideSearch && !this.state.hideSearch && (this.filters.s || data.items.length > 2)) {
                    searchPanel = R.div({ className: 'list-search' }, R.input({ ref: function (input) { _this.searchInput = input; }, className: 'list-search-input', placeholder: utils_js_1.L('SEARCH_LIST'), onChange: this.changeSearch, defaultValue: this.filters.s }), R.a({
                ...
                if (createButton || searchPanel) {
                    header = R.div({ className: 'list-header' }, createButton, searchPanel);
                }
            }
            var body;
            ...
            if (data.total > 0) {
                ...
                if (!body) {
                    ...
                    node.fields.some(function (field) {
                        ...
                        if (_this.isVisibleField(field)) {
                            tableHeader.push(R.td({ key: field.id, className: (field.fieldType === FIELD_2_INT) ? 'list-row-header list-row-header-num' : 'list-row-header' }, rowHeader, fieldAdmin));
                        }
                    });
                    ...
                    body = R.table({ className: 'list-table' }, R.thead(null, R.tr(null, tableHeader)), R.tbody({ className: 'list-body' }, lines));
                }
            }
                ...
                body = R.div({ className: 'list-emty' }, emptyIcon, R.div(null, t1), R.div(null, t2));
            }
            var pages = [];
            var recPerPage;
            ...
            return R.div({ className: 'list-container form-node-' + node.id }, nodeAdmin, title, header, footer, body, footer);
        };
        return List;
