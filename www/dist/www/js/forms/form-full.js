...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../fields/field-wrap.js", "../user.js", "../utils.js", "./form-tab.js", "./event-processing-mixins.js", "../admin/node-admin.js", "../loading-indicator.js"], factory);
    }
    ...
    var form_tab_js_1 = require("./form-tab.js");
    var event_processing_mixins_js_1 = require("./event-processing-mixins.js");
    ...
    var FormFull = /** @class */ (function (_super) {
        ...
        FormFull.prototype.render = function () {
            ...
            if (!node) {
                return R.div({ className: 'field-lookup-loading-icon-container' }, utils_js_1.renderIcon('cog fa-spin fa-2x'));
            }
            var tabs;
            var fields = [];
            ...
            if (!this.props.inlineEditable) {
                ...
                if (this.props.editable) {
                    if (!node.draftable || !isMainTab || this.disableDrafting || (data.id && !data.isP) || !(node.prevs & PREVS_PUBLISH)) {
                        saveButton = R.button({ className: 'clickable success-button save-btn', onClick: this.saveClick, title: utils_js_1.L('SAVE') }, this.isSlave() ? utils_js_1.renderIcon('check') : utils_js_1.renderIcon('floppy-o'), this.isSlave() ? '' : utils_js_1.L('SAVE'));
                    }
                    else {
                        if (data.status === 1) {
                            draftButton = R.button({ className: 'clickable default-button', onClick: function () { _this.saveClick(true); }, title: utils_js_1.L('UNPUBLISH') }, utils_js_1.L('UNPUBLISH'));
                            saveButton = R.button({ className: 'clickable success-button save-btn', onClick: this.saveClick }, utils_js_1.L('SAVE'));
                        }
                        else {
                            draftButton = R.button({ className: 'clickable default-button', onClick: function () { _this.saveClick(true); }, title: utils_js_1.L('SAVE_TEMPLATE') }, utils_js_1.L('SAVE_TEMPLATE'));
                            saveButton = R.button({ className: 'clickable success-button save-btn', onClick: this.saveClick, title: utils_js_1.L('PUBLISH') }, utils_js_1.L('PUBLISH'));
                        }
                    }
                }
                if (user_js_1.iAdmin()) {
                ...
                if (!this.props.isCompact) {
                    var headerContent = this.header || this.state.header || R.span(null, node.icon ? utils_js_1.renderIcon(node.icon) : undefined, node.singleName);
                    header = R.h4({ className: "form-header" }, headerContent);
                }
                if (this.props.editable) {
                    closeButton = R.button({ className: 'clickable default-button', onClick: this.cancelClick, title: utils_js_1.L('CANCEL') }, utils_js_1.renderIcon('caret-left'), this.isSlave() ? '' : utils_js_1.L('CANCEL'));
                }
                else {
                    closeButton = R.button({ className: 'clickable default-button', onClick: this.cancelClick }, utils_js_1.renderIcon('caret-left'), this.isSlave() ? '' : utils_js_1.L('BACK'));
                }
            }
            return R.div({ className: className }, nodeAdmin, header, tabs || fields, R.div({ className: (this.state.footerHidden || this.props.inlineEditable) ? 'form-footer hidden' : 'form-footer' }, deleteButton, draftButton, saveButton, closeButton));
        };
        return FormFull;
