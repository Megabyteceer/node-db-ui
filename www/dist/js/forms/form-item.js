...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "js/entry.js", "../fields/field-wrap.js", "../utils.js", "./form-mixins.js"], factory);
    }
    ...
    var renderItemsButtons = function (node, data, refreshFunction, formItem, editButtonFilters) {
        if (formItem && formItem.props.isLookup) {
            ...
            if (data.hasOwnProperty('isP') && (!formItem || !formItem.props.disableDrafting)) {
                if (data.status === 1) {
                    buttons.push(entry_js_1.R.button({ key: 1, className: 'clickable toolbtn unpublish-btn', title: utils_js_1.L('UNPUBLISH'), onClick: function () { publishClick(true, node, data).then(refreshFunction); } }, utils_js_1.renderIcon('eye')));
                }
                else {
                    buttons.push(entry_js_1.R.button({ key: 1, className: 'clickable toolbtn publish-btn', title: utils_js_1.L('PUBLISH'), onClick: function () { publishClick(false, node, data).then(refreshFunction); } }, utils_js_1.renderIcon('eye-slash')));
                }
            }
            if (editButtonFilters != 'noed') {
                if (data.hasOwnProperty('isE')) {
                    if (!formItem || !formItem.props.list || !formItem.props.list.state.noEditButton) {
                        buttons.push(entry_js_1.R.a({
                        ...
                        }, entry_js_1.R.button({ className: 'clickable toolbtn edit-btn', title: utils_js_1.L('EDIT', itemName) }, utils_js_1.renderIcon('pencil'))));
                    }
                }
                else if (!formItem || !formItem.props.list || !(formItem.props.list.state.noPreviewButton || formItem.props.list.props.noPreviewButton)) {
                    buttons.push(entry_js_1.R.a({ key: 2, href: utils_js_1.loactionToHash(node.id, data.id, undefined) }, entry_js_1.R.button({ className: 'clickable toolbtn view-btn', title: utils_js_1.L('DETAILS') + itemName }, utils_js_1.renderIcon('search'))));
                }
            }
            if (data.hasOwnProperty('isD')) {
                buttons.push(entry_js_1.R.button({
        ...
        FormItem.prototype.render = function () {
            ...
            for (var k in flds) {
                ...
                if (this.isVisibleField(field)) {
                    ...
                    fields.push(entry_js_1.R.td({ key: field.id, className: className }, React.createElement(field_wrap_js_1.default, { key: k, field: field, initialValue: data[field.fieldName], form: this, isCompact: true, isTable: true })));
                }
            }
            /** @type any */
            var itemProps = {};
            ...
            fields.push(entry_js_1.R.td({ key: 'b', className: 'form-item-row form-item-row-buttons' }, buttons, additionalButtons));
            return entry_js_1.R.tr(itemProps, fields);
        };
        return FormItem;
