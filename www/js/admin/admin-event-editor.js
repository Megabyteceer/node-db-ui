...
function admin_editSource(handler, node_, field) {
	...
	myAlert(React.createElement(AdminEventEditor, {type:type, title:title, handler:handler, itemId:id, node:node}), false, false, true);
}
...
function javascriptHint(cm){
	...
	if (list) {
		return {from: CodeMirror.Pos(cur.line, from), to: CodeMirror.Pos(cur.line, to), list: list};
	}
...
class AdminEventEditor extends React.Component {
				...
				extraKeys: {
					...
					"Ctrl-D": function (instance) {
						...
						if (!instance.doc.somethingSelected()) {
							...
							instance.doc.setCursor({line:c.line, ch:text.length});
							
							instance.replaceSelection('\r\n'+text);
							instance.doc.setCursor({line:c.line+1, ch:c.ch});
						} else {
	...
	render() {
		...
		var fields = this.props.node.fields.map(function(f, i){
			...
			return ReactDOM.div({key:i},
				((f.fieldType === FIELD_17_TAB)?ReactDOM.hr({className:'halfvisible'}):''),
		...
		return ReactDOM.div( {style:{textAlign:'left'}},
			ReactDOM.h4(null,this.props.title),
			ReactDOM.div({style:{margin:'30px'}},
